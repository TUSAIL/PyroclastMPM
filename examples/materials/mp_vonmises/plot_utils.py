...
def q_p_plot(stresses, file, title):
    ...
    # Hyrdostatic stress
    p_values = (1/3.)*(stresses[:,0,0] + stresses[:,1,1] + stresses[:,2,2])  # hydrostatic pressure

    # Deviatoric stress
    dev_stress_values = stresses - np.identity(3)*p_values[:,None,None] # deviatoric stress
