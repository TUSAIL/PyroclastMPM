...
// Functions to test
// [x] Gravity::Gravity, no ramp (implicitly tested in apply_on_nodes_f_ext, no ramp)
// [x] Gravity::Gravity, with ramp (implicitly tested in apply_on_nodes_f_ext, with ramp)
// [x] Gravity::apply_on_nodes_f_ext, no ramp
...
TEST(Gravity, APPLY_GRAVITY_WITH_RAMP)
{
  ...
  // this might be needed if we add particles to the apply_on_nodes_f_ext argument later (for NGF?)
  // ParticlesContainer particles = ParticlesContainer(std::vector({Vectorr::Ones()*0.1}));
