...
// [ ] PlanarDomain::apply_on_particles(touching the boundary and not touching the boundary)
// [ ] PlanarDomain::apply_on_particles (friction and not)
...
TEST(PlanarDomain, apply_on_particles)
{
    ...
    const std::vector<Vectorr> pos = {
        Vectorr({0.1, 0.1, 0.0}), Vectorr({0.199, 0.1, 0.0}), Vectorr({0.82, 0.0, 1.}), Vectorr({0.82, 0.6, 0.})};
#elif DIM == 2
    const std::vector<Vectorr> pos = {
        Vectorr({0.1, 0.1}), Vectorr({0.199, 0.1}), Vectorr({0.82, 0.0}), Vectorr({0.82, 0.6})};
#else
