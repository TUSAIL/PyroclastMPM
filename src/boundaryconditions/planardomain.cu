...
#include "planardomain_inline.cuh"
  ...
  struct ApplyPlanarDomain
  {
    ...
    ApplyPlanarDomain(
        ...
        const Vectorr _axis1_friction) : grid_start(_grid_start),
                                         grid_end(_grid_end),
                                         axis0_friction(_axis0_friction),
                                         axis1_friction(_axis1_friction){};
...
#pragma unroll
      for (int i = 0; i < DIM; i++)
      {
        if (overlap0[i] > 0)
        {
          const Vectorr vel_depth = (overlap0[i] * normals0[i]).dot(velocity) * normals0[i];
          const Vectorr fric_term = normals0[i] - axis0_friction(i) * (velocity - normals0[i].dot(velocity) * normals0[i]);
...
#pragma unroll
      for (int i = 0; i < DIM; i++)
      {
        if (overlap1[i] > 0)
        {
          const Vectorr vel_depth = (overlap1[i] * normals1[i]).dot(velocity) * normals1[i];
          const Vectorr fric_term = normals1[i] - axis1_friction(i) * (velocity - normals1[i].dot(velocity) * normals1[i]);
