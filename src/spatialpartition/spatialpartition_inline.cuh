...
__device__ __host__ inline void calculate_hashes(Vectori *bins_gpu,
...
{
    const Vectorr relative_position = (positions_gpu[tid] - grid_start) * inv_cell_size;
    bins_gpu[tid] = relative_position.cast<int>();
    hashes_unsorted_gpu[tid] = NODE_MEM_INDEX(bins_gpu[tid], num_cells); // MACRO defined in type_commons.cuh
}
